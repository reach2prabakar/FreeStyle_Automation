...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:function name="testng:substring">
        <xsl:param name="arg"/>
        <xsl:sequence select="replace(replace($arg,'\s+$',''),'^\s+','')"/>
    </xsl:function>
                        ...
                        <table width="100%" cellpadding="5" cellspacing="1">
                           <thead>
                           	<tr style="background-color: #eaf0f7;">
                           		<td width="100%"> <div class="{testng:suiteStateClass($testCaseElements)}"/>Test Suite Name</td>
                           		<td style="background-color: #FFBBBB;" align="center">FAIL</td>
                           		<td style="background-color: lightgreen;" align="center">PASS</td>
                           		<td style="background-color: #FFFFBB;" align="center">SKIP</td>
            ...
            <tr id="{concat($methodId, '_row')}" class="{testng:testMethodStatus(.)}">
                ...
                <td width="100%" class="firstMethodCell">
                    <a onclick="toggleDetailsVisibility('{$detailsId}')">
                        <xsl:value-of select="(testng:trim(testng:concatParams(./params)))"/>
                    </a>
                </td>
                <td nowrap="true">
